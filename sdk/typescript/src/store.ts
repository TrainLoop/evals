...
// eslint-disable-next-line @typescript-eslint/no-implied-eval
const path: typeof import("path") | null = isNode
  ? eval("require")("path")
  : null;
...
/* ------------------------------------------------------------------ */
/*  Storage detection and imports                                     */
/* ------------------------------------------------------------------ */

let useFlystorage = false;
let flystorageModule: any = null;

// Try to import flystorage modules and detect if they're available
try {
  // Dynamic import to avoid bundling issues
  const flystorage = eval("require")("@flystorage/file-storage");
  const localAdapter = eval("require")("@flystorage/local-fs");
  const s3Adapter = eval("require")("@flystorage/aws-s3");
  const gcsAdapter = eval("require")("@flystorage/google-cloud-storage");

  if (flystorage && localAdapter && s3Adapter && gcsAdapter) {
    useFlystorage = true;
    flystorageModule = { flystorage, localAdapter, s3Adapter, gcsAdapter };
    logger.debug(
      "Flystorage modules detected, using flystorage implementation"
    );
  }
} catch (error) {
  logger.debug(
    "Flystorage modules not available, falling back to native fs implementation"
  );
  useFlystorage = false;
}

function isCloudStoragePath(dataDir: string): boolean {
  return (
...
export function updateRegistry(
  ...
  // If it's a cloud storage path and flystorage is available, use async storage
  if (isCloudStoragePath(dataDir) && useFlystorage) {
    logger.debug("Using flystorage for cloud storage registry update");

    // Import the async storage module
    import("./storage")
      .then((storage) => {
        storage.updateRegistry(dataDir, location, tag).catch((error) => {
          logger.error(`Failed to update registry with flystorage: ${error}`);
        });
      })
      .catch((error) => {
        logger.error(`Failed to import storage module: ${error}`);
      });
    return;
  }

  // Use original Node.js implementation for local paths or when flystorage is not available
  if (!isNode || !fs || !path) {
 ...
 */
export function saveSamples(dataDir: string, samples: CollectedSample[]): void {
  // If it's a cloud storage path and flystorage is available, use async storage
  if (isCloudStoragePath(dataDir) && useFlystorage) {
    logger.debug("Using flystorage for cloud storage sample save");

    // Import the async storage module
    import("./storage")
      .then((storage) => {
        storage.saveSamples(dataDir, samples).catch((error) => {
          logger.error(`Failed to save samples with flystorage: ${error}`);
        });
      })
      .catch((error) => {
        logger.error(`Failed to import storage module: ${error}`);
  ...
  try {
    ...
    if (files.length > 0) {
      ...
      if (age < windowTime) {
        ...
        logger.debug(
          `File too old (${age}ms > ${windowTime}ms), using new timestamp`
        );
      }
    } else {
...
function loadRegistry(filePath: string): Registry {
  ...
  try {
    ...
    logger.debug(
      `Successfully loaded registry with ${
        Object.keys(registry.files || {}).length
      } files`
    );
    return registry;
  } catch (error) {
