...
The Studio server reads evaluation results from the configured data folder. Ensure `TRAINLOOP_DATA_FOLDER` points to a valid path and that the folder exists. The config cascade matches the [`trainloop eval`](./eval) command. See [Environment Variables](./env-vars) for details.
