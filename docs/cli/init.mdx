...
The command prepares a workspace under `trainloop/` so you can start collecting events and writing metrics immediately. The data folder defaults to `trainloop/data/`. You can override it later using the [`TRAINLOOP_DATA_FOLDER`](./cli-env-vars#trainloop_data_folder) environment variable.
